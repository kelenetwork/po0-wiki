...
func TestAgentPollReportUpdatesPublicSnapshot(t *testing.T) {
	...
	if !strings.Contains(rec.Body.String(), "AGENT_ID='src-shanghai-ctc'") || !strings.Contains(rec.Body.String(), "TOKEN='"+created.Token+"'") || !strings.Contains(rec.Body.String(), "HUB_URL='https://wiki.uuuz.de/api/agent'") || !strings.Contains(rec.Body.String(), "ENABLE_ICMP=true") || strings.Contains(rec.Body.String(), "INSTALL_TOKEN") || strings.Contains(rec.Body.String(), "/api/install/") {
