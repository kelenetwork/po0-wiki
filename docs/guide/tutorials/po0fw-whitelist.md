...
Po0 机器默认开启「防火墙白名单」：**只有白名单里的来源 IP 才能访问入站端口**。家宽 IP 经常变，手动加白很快就会烦。本教程教你用 `po0fw` 脚本让设备**自动**把自己的出口 IP 加进白名单，支持：
...
不会。脚本用三个公共 IP 源（`ip.sb` / `icanhazip` / `ipify`）探测真实 IPv4 出口，请求强制 IPv4。如果你的代理接管了全部流量，请把这三个域名和 `console.po0.io` 设为直连规则。
