...
type Check = { id: string; display_name: string; source_id: string; target_id: string; tags: string[]; status: string; latency_ms: number; loss_pct: number; jitter_ms: number; interval_seconds: number; enabled: boolean; updated_at: string };
...
type InstallPayload = { agent_id: string; token: string; hub_url: string; systemd_unit: string; config_json: string; install_command: string };
...
const uninstallCommand = `sudo systemctl disable --now wiki-probe-agent.service || true
...
function normalizeKind(kind?: string): TargetKind { return kind === 'icmp' || kind === 'http' ? kind : 'tcp'; }
function normalizePath(kind: TargetKind, path?: string) { if (kind !== 'http') return ''; const next = (path || '/').trim() || '/'; return next.startsWith('/') ? next : `/${next}`; }
...
function formatTargetAddress(target: Target) {
  ...
  if (kind === 'http') {
    ...
    const path = normalizePath('http', target.path);
    return `${scheme}://${target.host}:${target.port || 443}${path}`;
  }
...
export default function AdminApp({ page }: AdminAppProps) {
  ...
  async function loadAll() {
    ...
    setSources(sourceData.sources ?? []); setTargets((targetData.targets ?? []).map((item) => ({ ...item, kind: normalizeKind(item.kind), path: normalizePath(normalizeKind(item.kind), item.path) }))); setChecks(checkData.checks ?? []); setAgents(agentData.agents ?? []);
  }
  ...
  function targetForm(item?: Target): TargetForm { const kind = normalizeKind(item?.kind); return item ? { id: item.id, name: '', display_name: item.display_name, region: item.region, tags: tagsToText(item.tags), kind, host: item.host || '', port: String(item.port || (kind === 'http' ? 443 : 443)), path: normalizePath(kind, item.path) || '/' } : blankTarget; }
