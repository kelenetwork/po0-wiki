  ...
  return (
    <header className="po0-shell__nav">
      <a className="po0-shell__brand" href="/">
        ...
        <span>Po0</span><i /><span>Wiki</span>
      </a>
      <nav>
      ...
      <a className="po0-shell__cta" href="/admin/sources" aria-label="控制台" data-current={current}><span /></a>
    </header>
  );
}
...
type Target = Source & { kind: TargetKind; host: string; port: number; path: string; show_in_lg: boolean };
...
type TargetForm = SourceForm & { kind: TargetKind; host: string; port: string; path: string; show_in_lg: boolean };
...
const blankTarget: TargetForm = { ...blankSource, kind: 'tcp', host: '', port: '443', path: '/', show_in_lg: true };
...
function metric(check: Check) { return `${check.latency_ms ?? 0}ms / loss ${check.loss_pct ?? 0}% / jitter ${check.jitter_ms ?? 0}ms`; }
function maskToken(token: string) { return token.length <= 12 ? `${token.slice(0, 3)}…` : `${token.slice(0, 8)}…${token.slice(-4)}`; }
function displayAgentToken(agent?: Agent, visible = false) { if (!agent?.token) return '— 重置以生成 Token'; return visible ? agent.token : maskToken(agent.token); }
...
export default function AdminApp({ page }: AdminAppProps) {
  ...
  useEffect(() => {
    ...
    return () => { delete document.documentElement.dataset.admin; delete document.body.dataset.pageType; };
  }, []);
  ...
  async function adminFetch<T>(path: string, init: RequestInit = {}): Promise<T> {
    ...
    try { message = (JSON.parse(text) as { error?: string }).error || message; } catch (_) { /* keep text */ }
  ...
  async function loadAll() {
    ...
    setTargets((targetData.targets ?? []).map((item) => ({ ...item, kind: normalizeKind(item.kind), path: normalizePath(normalizeKind(item.kind), item.path), show_in_lg: item.show_in_lg !== false })));
    setChecks((checkData.checks ?? []).map((item) => ({ ...item, last_error: item.last_error ?? '' })));
  ...
  function targetForm(item?: Target): TargetForm {
    ...
    return item ? { id: item.id, name: '', display_name: item.display_name, region: item.region, tags: tagsToText(item.tags), kind, host: item.host || '', port: String(item.port || (kind === 'http' ? 80 : 443)), path: normalizePath(kind, item.path) || '/', show_in_lg: item.show_in_lg !== false } : { ...blankTarget };
  }
  function checkForm(item?: Check, source?: Source, target?: Target): CheckForm {
    if (item) return { id: item.id, name: '', display_name: item.display_name, source_id: item.source_id, target_id: item.target_id, tags: tagsToText(item.tags), interval_seconds: String(item.interval_seconds || 30), enabled: item.enabled };
  ...
  async function saveSource(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await loadAll(); setNotice(drawer.mode === 'edit' ? '源节点已更新。' : '源节点已创建。');
      if (drawer.mode === 'create') await createAgentAndOpenInstall(created.id, `${created.display_name} 接入命令`); else setDrawer(null);
    } catch (error) { showError(error); }
  }
  async function saveTarget(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!drawer || drawer.kind !== 'target') return;
    const form = drawer.form; const kind = normalizeKind(form.kind);
    const body = JSON.stringify({ name: form.name || form.display_name, display_name: form.display_name || form.name, region: form.region, tags: tagsFromText(form.tags), kind, host: form.host, port: kind === 'icmp' ? 0 : Number(form.port || (kind === 'http' ? 80 : 443)), path: normalizePath(kind, form.path), status: 'online', show_in_lg: form.show_in_lg });
  ...
  async function saveSourceOrder(ids: string[]) { await adminFetch('/api/admin/sources/order', { method: 'POST', body: JSON.stringify({ ids }) }); setNotice('源节点排序已保存。'); }
  async function saveTargetOrder(ids: string[]) { await adminFetch('/api/admin/targets/order', { method: 'POST', body: JSON.stringify({ ids }) }); setNotice('目标排序已保存。'); }
  async function handleSourceReorder(from: number, to: number) { if (from === to) return; const next = reorderItems(sources, from, to); setSources(next); try { await saveSourceOrder(next.map((item) => item.id)); await loadAll(); } catch (error) { showError(error); await loadAll(); } }
  async function handleTargetReorder(from: number, to: number) { if (from === to) return; const next = reorderItems(targets, from, to); setTargets(next); try { await saveTargetOrder(next.map((item) => item.id)); await loadAll(); } catch (error) { showError(error); await loadAll(); } }
  ...
  return (
    <div className="po0-shell">
      <AdminTopBar current="admin" />
      <section className="admin-layout">
        <aside className="admin-sidebar">
          <a className="admin-brand" href="/admin/sources">Probe Admin</a>
          <nav>{navItems.map((item) => <a key={item.page} className={normalizedPage === item.page ? 'active' : ''} href={item.href}>{item.label}</a>)}<button type="button" onClick={logout}>退出登录</button></nav>
        </aside>
        <main className="admin-shell">
          <header className="admin-header"><div><p className="admin-kicker">Admin Console</p><h1>{pageTitles[normalizedPage]}</h1></div><div className="admin-auth"><span>{maskToken(token)}</span><button type="button" onClick={logout}>退出</button></div></header>
          <Toast notice={notice} /><ErrorBanner message={errorBanner} onClose={() => setErrorBanner('')} />
          {normalizedPage === 'home' && <Dashboard sources={sources} targets={targets} checks={checks} />}
          {normalizedPage === 'sources' && <SourcesPage sources={sources} agents={agentBySource} visibleTokens={visibleTokens} onReorder={handleSourceReorder} onToggleToken={(id) => setVisibleTokens((items) => ({ ...items, [id]: !items[id] }))} onNew={() => setDrawer({ kind: 'source', mode: 'create', title: '新增源节点', form: sourceForm() })} onEdit={(item) => setDrawer({ kind: 'source', mode: 'edit', title: '编辑源节点', form: sourceForm(item) })} onDelete={(id) => deleteResource('sources', id)} onReset={resetToken} onInstall={showInstall} onUninstall={copyUninstall} onCopy={copyText} />}
          {normalizedPage === 'targets' && <TargetsPage targets={targets} onReorder={handleTargetReorder} onNew={() => setDrawer({ kind: 'target', mode: 'create', title: '新增目标', form: targetForm() })} onEdit={(item) => setDrawer({ kind: 'target', mode: 'edit', title: '编辑目标', form: targetForm(item) })} onDelete={(id) => deleteResource('targets', id)} />}
          {normalizedPage === 'checks' && <ChecksPage view={checksView} setView={setChecksView} sources={sources} targets={targets} checks={checks} sourceName={sourceName} targetName={targetName} checkByPair={checkByPair} onNew={() => setDrawer({ kind: 'check', mode: 'create', title: '新增探测任务', form: checkForm() })} onCreatePair={(source, target) => setDrawer({ kind: 'check', mode: 'create', title: '新增探测任务', form: checkForm(undefined, source, target) })} onEdit={(item) => setDrawer({ kind: 'check', mode: 'edit', title: '编辑探测任务', form: checkForm(item), deletable: true })} />}
        </main>
      </section>
...
function DragHandle({ index, onReorder }: { index: number; onReorder: (from: number, to: number) => void }) {
  function onDragStart(event: DragEvent<HTMLButtonElement>) { event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', String(index)); }
  function onDrop(event: DragEvent<HTMLButtonElement>) { event.preventDefault(); const from = Number(event.dataTransfer.getData('text/plain')); if (Number.isFinite(from)) onReorder(from, index); }
  return <button type="button" className="admin-drag-handle" draggable onDragStart={onDragStart} onDragOver={(event) => event.preventDefault()} onDrop={onDrop} title="拖拽排序">↕</button>;
}
function SourcesPage({ sources, agents, visibleTokens, onToggleToken, onNew, onEdit, onDelete, onReset, onInstall, onUninstall, onCopy, onReorder }: { sources: Source[]; agents: Map<string, Agent>; visibleTokens: Record<string, boolean>; onToggleToken: (id: string) => void; onNew: () => void; onEdit: (item: Source) => void; onDelete: (id: string) => void; onReset: (id: string) => void; onInstall: (id: string) => void; onUninstall: (id: string) => void; onCopy: (text: string) => void; onReorder: (from: number, to: number) => void }) {
  return <><Toolbar title="源节点" action="新增源节点" onAction={onNew} hint="拖拽 ↕ 可调整前台展示顺序" /><TableWrap><table><thead><tr><th>排序</th><th>名称</th><th>地区</th><th>标签</th><th>状态</th><th>最近活动时间</th><th className="admin-actions">操作</th></tr></thead><tbody>{sources.map((item, index) => { const agent = agents.get(item.id); return <RowGroup key={item.id} main={<tr><td><DragHandle index={index} onReorder={onReorder} /></td><td><strong>{empty(item.display_name)}</strong><small>{item.id}</small></td><td>{empty(item.region)}</td><td><Chips tags={item.tags} /></td><td>{statusLabel(item.status)}</td><td>{formatTime(item.updated_at)}</td><td className="admin-actions"><button type="button" onClick={() => onEdit(item)}>编辑</button><button type="button" onClick={() => onReset(item.id)}>重置 Token</button><button type="button" onClick={() => onInstall(item.id)}>安装</button><button type="button" onClick={() => onUninstall(item.id)}>卸载</button><button type="button" className="danger" onClick={() => onDelete(item.id)}>删除</button></td></tr>} detail={<tr className="admin-detail"><td /><td colSpan={6}><div className="admin-token-row"><span>Agent Token</span><code>{displayAgentToken(agent, visibleTokens[item.id])}</code><button type="button" onClick={() => onToggleToken(item.id)}>{visibleTokens[item.id] ? '隐藏' : '显示'}</button><button type="button" onClick={() => onCopy(agent?.token || '')} disabled={!agent?.token}>复制</button><span>最后上报 {formatTime(agent?.last_reported_at)}</span></div></td></tr>} />; })}</tbody></table></TableWrap></>;
}
function TargetsPage({ targets, onNew, onEdit, onDelete, onReorder }: { targets: Target[]; onNew: () => void; onEdit: (item: Target) => void; onDelete: (id: string) => void; onReorder: (from: number, to: number) => void }) {
  return <><Toolbar title="目标" action="新增目标" onAction={onNew} hint="LG 开关只影响 Looking Glass，不影响状态页" /><TableWrap><table><thead><tr><th>排序</th><th>名称</th><th>地区</th><th>协议</th><th>地址</th><th>LG</th><th>标签</th><th>状态</th><th>最近活动时间</th><th className="admin-actions">操作</th></tr></thead><tbody>{targets.map((item, index) => <tr key={item.id}><td><DragHandle index={index} onReorder={onReorder} /></td><td><strong>{empty(item.display_name)}</strong><small>{item.id}</small></td><td>{empty(item.region)}</td><td>{normalizeKind(item.kind)}</td><td>{formatTargetAddress(item)}</td><td><span className={`admin-lg-pill ${item.show_in_lg ? 'on' : 'off'}`}>{item.show_in_lg ? '显示' : '隐藏'}</span></td><td><Chips tags={item.tags} /></td><td>{empty(item.status)}</td><td>{formatTime(item.updated_at)}</td><td className="admin-actions"><button type="button" onClick={() => onEdit(item)}>编辑</button><button type="button" className="danger" onClick={() => onDelete(item.id)}>删除</button></td></tr>)}</tbody></table></TableWrap></>;
}
function ChecksPage({ view, setView, sources, targets, checks, sourceName, targetName, checkByPair, onNew, onCreatePair, onEdit }: { view: 'matrix' | 'list'; setView: (view: 'matrix' | 'list') => void; sources: Source[]; targets: Target[]; checks: Check[]; sourceName: Map<string, string>; targetName: Map<string, string>; checkByPair: Map<string, Check>; onNew: () => void; onCreatePair: (source: Source, target: Target) => void; onEdit: (item: Check) => void }) { return <><Toolbar title="探测任务" action="新增任务" onAction={onNew} /><div className="admin-toggle"><button type="button" className={view === 'matrix' ? 'active' : ''} onClick={() => setView('matrix')}>矩阵</button><button type="button" className={view === 'list' ? 'active' : ''} onClick={() => setView('list')}>列表</button></div>{view === 'matrix' ? <ChecksMatrix sources={sources} targets={targets} checkByPair={checkByPair} onCreatePair={onCreatePair} onEdit={onEdit} /> : <ChecksList checks={checks} sourceName={sourceName} targetName={targetName} onEdit={onEdit} />}</>; }
function ChecksMatrix({ sources, targets, checkByPair, onCreatePair, onEdit }: { sources: Source[]; targets: Target[]; checkByPair: Map<string, Check>; onCreatePair: (source: Source, target: Target) => void; onEdit: (item: Check) => void }) { return <div className="admin-matrix"><table><thead><tr><th>源 / 目标</th>{targets.map((target) => <th key={target.id}>{target.display_name}</th>)}</tr></thead><tbody>{sources.map((source) => <tr key={source.id}><th>{source.display_name}<small>{source.id}</small></th>{targets.map((target) => { const check = checkByPair.get(`${source.id}::${target.id}`); return <td key={target.id}>{check ? <button type="button" className={`matrix-cell status-${check.status}`} onClick={() => onEdit(check)}><strong>{check.display_name}</strong><span>{metric(check)}</span></button> : <button type="button" className="matrix-empty" onClick={() => onCreatePair(source, target)}>＋</button>}</td>; })}</tr>)}</tbody></table></div>; }
function ChecksList({ checks, sourceName, targetName, onEdit }: { checks: Check[]; sourceName: Map<string, string>; targetName: Map<string, string>; onEdit: (item: Check) => void }) { return <TableWrap><table><thead><tr><th>名称</th><th>源节点</th><th>目标</th><th>轮询</th><th>启用</th><th>指标</th><th>最近错误</th><th className="admin-actions">操作</th></tr></thead><tbody>{checks.map((item) => <tr key={item.id}><td><strong>{item.display_name}</strong><small>{item.id}</small></td><td>{sourceName.get(item.source_id) || item.source_id}</td><td>{targetName.get(item.target_id) || item.target_id}</td><td>{item.interval_seconds}s</td><td>{item.enabled ? '是' : '否'}</td><td>{metric(item)}</td><td className="admin-last-error">{empty(item.last_error)}</td><td className="admin-actions"><button type="button" onClick={() => onEdit(item)}>编辑</button></td></tr>)}</tbody></table></TableWrap>; }
function Toolbar({ title, action, onAction, hint }: { title: string; action: string; onAction: () => void; hint?: string }) { return <div className="admin-toolbar"><div><h2>{title}</h2>{hint && <small>{hint}</small>}</div><button type="button" onClick={onAction}>{action}</button></div>; }
